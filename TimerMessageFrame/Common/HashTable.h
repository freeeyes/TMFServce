...
public:
    ...
    T* Pop_Uint32(uint32& u4Pos)
    {
        ...
        if (NULL == m_lpTable)
        {
            ...
            {
                PRINTF("[Pop_Uint32]m_lpTable is NULL.\n");
            }
        ...
        for (int32 i = m_nCurrLinkIndex; i < m_objHashPool.Get_Count(); i++)
        {
            if (m_lpTable[i] != NULL)
            {
                ...
                if (HASH_DEBUG_ON == m_emHashDebug)
                {
                    PRINTF("[Pop]1 index=%d, Del_Hash_Data(%d), Currused=%d pT=0x%08x.\n", m_nCurrLinkIndex, nRet, Get_Used_Count(), pT);
                }
        ...
        for (int32 i = 0; i < m_nCurrLinkIndex; i++)
        {
            if (m_lpTable[i] != NULL)
            {
                ...
                if (HASH_DEBUG_ON == m_emHashDebug)
                {
                    PRINTF("[Pop]2 index=%d, Del_Hash_Data(%d), Currused=%d, pT=0x%08x.\n", m_nCurrLinkIndex, nRet, Get_Used_Count(), pT);
                }
        ...
        if (HASH_DEBUG_ON == m_emHashDebug)
        {
            PRINTF("[Pop]2 index=%d, no Find, Currused=%d.\n", m_nCurrLinkIndex, Get_Used_Count());
        }
    ...
    bool Push(const char* pKey, T* pT)
    {
        if(NULL == m_lpTable)
        {
            ...
            if (HASH_DEBUG_ON == m_emHashDebug)
            {
                PRINTF("[Push] m_lpTable is NULL, pKey=%s, pT=0x%08x.\n", pKey, pT);
            }
        ...
        if (HASH_DEBUG_ON == m_emHashDebug)
        {
            PRINTF("[Push] index =%d, pKey=%s, pT=0x%08x.\n", uHashStart, pKey, pT);
        }
        ...
        if(NULL == pData)
        {
            if (HASH_DEBUG_ON == m_emHashDebug)
            {
                PRINTF("[Push]m_objHashPool pData = NULL.\n");
            }
        ...
        if(NULL == pLink)
        {
            if (HASH_DEBUG_ON == m_emHashDebug)
            {
                PRINTF("[Push]m_objHashLinkPool pData = NULL.\n");
            }
        ...
        if (HASH_DEBUG_ON == m_emHashDebug)
        {
            PRINTF("[Push] index =%d, pKey=%s, Currused=%d, pT=0x%08x OK.\n", m_emHashDebug, pKey, Get_Used_Count(), pT);
        }
        ...
        for(int32 i = 0; i < m_objHashPool.Get_Count(); i++)
        {
            if(NULL != m_lpTable[i])
            {
                ...
                while(NULL != pLastLink)
                {
                    ...
                    {
                        PRINTF("[CHashTable::Get_All_Used]pLastLink->m_pData is NULL.\n");
                    }
        ...
        for (int32 i = 0; i < m_objHashPool.Get_Count(); i++)
        {
            if (NULL != m_lpTable[i])
            {
                ...
                while (NULL != pLastLink)
                {
                    ...
                    {
                        PRINTF("[CHashTable::Get_All_Used_Key]pLastLink->m_pData is NULL.\n");
                    }
    ...
    void Clear()
    {
        ...
        for (int32 i = 0; i < m_objHashPool.Get_Count(); i++)
        {
            if (NULL != m_lpTable[i])
            {
                ...
                while (NULL != pLastLink)
                {
                    ...
                    {
                        PRINTF("[CHashTable::Clear]pLastLink->m_pData is NULL.\n");
                    }
        ...
        for (int32 i = 0; i < (int32)vecList.size(); i++)
        {
            ...
            {
                PRINTF("[CHashTable::Clear](%s) is Delete error.\n", vecList[i].c_str());
            }
        }
    }
    ...
    void Clear_Unit32()
    {
        ...
        if (HASH_DEBUG_ON == m_emHashDebug)
        {
            PRINTF("[CHashTable::Clear_Unit32](%d)Data is Clear.\n", Get_Used_Count());
        }
        ...
        for (int32 i = 0; i < (int32)vecList.size(); i++)
        {
            ...
            {
                PRINTF("[CHashTable::Clear_Unit32](%d) is Delete error.\n", vecList[i]);
            }
        }
    }
    ...
    int32 Add_Hash_Data_By_Key_Unit32(uint32 u4Key, T* pValue)
    {
        if (NULL == m_lpTable)
        {
            ...
            if (HASH_DEBUG_ON == m_emHashDebug)
            {
                PRINTF("[Add_Hash_Data]m_lpTable is NULL.\n");
            }
    ...
    int32 Add_Hash_Data(const char* pKey, T* pValue)
    {
        ...
        if((short)strlen(pKey) >= m_objHashPool.Get_Key_Length())
        {
            ...
            if (HASH_DEBUG_ON == m_emHashDebug)
            {
                PRINTF("[Add_Hash_Data](short)strlen(pKey) >= m_objHashPool.Get_Key_Length().\n");
            }
        ...
        else
        {
            if(NULL == m_lpTable[nPos])
            {
                ...
                else
                {
                    ...
                    if (HASH_DEBUG_ON == m_emHashDebug)
                    {
                        PRINTF("[Test]Add 1 pKey=%s, pData->m_pKey=%s.\n", pKey, pData->m_pKey);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    if (HASH_DEBUG_ON == m_emHashDebug)
                    {
                        PRINTF("[Test]Add 2 pKey=%s, pData->m_pKey=%s.\n", pKey, pData->m_pKey);
                    }
    ...
    int32 DelHashTablePos(const char* lpszString)
    {
        ...
        else
        {
            ...
            while (NULL != pLastLink)
            {
                if (HASH_DEBUG_ON == m_emHashDebug)
                {
                    PRINTF("[Test]lpszString=%s, pLastLink->m_pData->m_pKey=%s.\n", lpszString, pLastLink->m_pData->m_pKey);
                }
